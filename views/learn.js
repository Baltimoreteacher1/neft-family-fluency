// Monday: the strategy explainer.
//
// Animated with CSS and SVG, never a video file: a video is megabytes the
// family pays for, will not play inline on some Android browsers, and cannot
// be translated without a second recording. A built explainer respects
// prefers-reduced-motion for free, because the animation is CSS.
...
export async function render(container, app, match) {
  ...
  mount(container,
    ...
    el('div', { class: 'card' }, explainer(week.strategy.id)),
    el('div', { class: 'row' },
      el('button', { class: 'btn', onClick: () => app.go(`week/${weekNumber}/practice`) },
        t('week.practice')),
      el('button', { class: 'btn btn--ghost', onClick: () => app.go(`week/${weekNumber}`) },
        t('app.back')),
    ),
  );
}
